locals {
  ...
  qa_vault = "${var.env == "perftest" || var.env == "ithc" ? "infra-vault-qa" : ""}"
  vaultName       = "${format("%s%s%s%s%s", local.prod_vault, local.nonprod_vault, local.sandbox_vault, local.hmctsdemo_vault, local.qa_vault)}"
}
